...
const newMain = `<main className="flex-grow max-w-[1400px] mx-auto w-full pb-28 md:pb-10 relative">
      ...
      {!activeSection && (
        <nav className="fixed bottom-0 w-full bg-white/90 backdrop-blur-xl border-t border-slate-200 pb-safe z-50 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] text-xs font-semibold">
          <div className="flex flex-row justify-around items-center h-20 max-w-lg mx-auto px-2 relative font-khmer flex-nowrap">
            <NavItem 
              icon={<Home />} 
              label="ទំព័រដើម" 
...
function NavItem({ icon, label, isActive, onClick }: { icon: React.ReactElement, label: string, isActive: boolean, onClick: () => void }) {
  return (
    <button onClick={onClick} className={\`flex flex-col items-center justify-center gap-1 w-16 shrink-0 transition-colors \${isActive ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600'}\`}>
    ...
    <div className="space-y-8 pb-10">
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-4">
          <MenuCard actionColor="bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100" title="វត្តមានស្កេន (QR)" icon={<QrCode className="w-8 h-8" />} onClick={() => onSelectSection('attendance')} />
          <MenuCard actionColor="bg-indigo-50 text-indigo-700 border-indigo-200 hover:bg-indigo-100" title="ជំនួយការគ្រូ AI" icon={<Sparkles className="w-8 h-8" />} onClick={() => onSelectSection('classroom-tools')} />
          <MenuCard actionColor="bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-100" title="តារាងសម្រង់ពិន្ទុ" icon={<FileSpreadsheet className="w-8 h-8" />} onClick={() => onSelectSection('grade-summary')} />
          <MenuCard actionColor="bg-teal-50 text-teal-700 border-teal-200 hover:bg-teal-100" title="ទំនាក់ទំនងមាតាបិតា" icon={<PhoneCall className="w-8 h-8" />} onClick={() => onSelectSection('parent-comm')} />
        </div>
      </section>
      ...
      {atRiskCount > 0 && (
        ...
        >
          ...
          <div>
            <h4 className="text-lg font-bold font-kantumruy mb-1">មានសិស្សតម្រូវការពិសេស ({atRiskCount})</h4>
            <p className="text-rose-100 text-sm px-2">សិស្សមួយចំនួនកំពុងត្រូវការការគាំទ្រផ្នែកពិន្ទុ និងអំណានបន្ថែម។</p>
          </div>
          <button 
...
function ClassTab({ onSelectSection }: { onSelectSection: (s: string) => void }) {
  return (
    <div className="space-y-6 pb-10">
      <h3 className="text-2xl font-bold font-kantumruy text-slate-800">ថ្នាក់រៀន និងសិស្ស</h3>
...
function LibraryTab({ onSelectSection }: { onSelectSection: (s: string) => void }) {
  return (
    <div className="space-y-6 pb-10">
      <h3 className="text-2xl font-bold font-kantumruy text-slate-800">ធនធានសិក្សា</h3>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
...
function MoreTab({ onSelectSection }: { onSelectSection: (s: string) => void }) {
  return (
    <div className="space-y-6 pb-10">
      <h3 className="text-2xl font-bold font-kantumruy text-slate-800">ផ្សេងៗ</h3>
...
function BannerItem({ title, subtitle, icon, onClick, color = "text-slate-800 bg-white border-slate-200" }: { title: string, subtitle: string, icon: React.ReactElement, onClick: () => void, color?: string }) {
  return (
    <button onClick={onClick} className={\`w-full text-left p-5 rounded-[20px] border shadow-sm hover:shadow-md transition-all flex items-center gap-4 \${color}\`}>
      <div className="w-12 h-12 bg-black/5 rounded-xl flex items-center justify-center shrink-0">
        {React.cloneElement(icon, { className: 'w-6 h-6' })}
      </div>
      <div>
        <h4 className="font-bold font-kantumruy text-[15px]">{title}</h4>
        <p className="text-sm mt-0.5 opacity-70">{subtitle}</p>
      </div>
...
const newMenuCardBody = `className={\`relative group \${actionColor ? 'p-5 rounded-3xl border ' + actionColor : 'glass-card p-6 rounded-[24px]'} flex flex-col items-center justify-center text-center \${actionColor ? 'gap-3 aspect-auto h-[120px]' : 'gap-5 aspect-square'} transition-all duration-500 cursor-pointer overflow-hidden w-full\`}>
      ...
      <span className={\`relative z-10 font-bold font-kantumruy text-sm \${actionColor ? ' leading-snug px-2' : 'text-slate-700 group-hover:text-white transition-colors duration-500 leading-snug'}\`}>
        {title}
      </span>
    </motion.button>`;
